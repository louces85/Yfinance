...
function create_data_base(){
    $docker_exec "CREATE TABLE if not exists history ( id SERIAL PRIMARY KEY, ticker VARCHAR(7) NOT null UNIQUE, price_min float8 null, price_max float8 null, volume_min float8 null, volume_max float8 null, net_income boolean null, date_update date null );"
